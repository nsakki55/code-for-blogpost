...
resource "aws_ecs_task_definition" "prod-prefect-agent" {
  family                   = "prefect2-agent"
  network_mode             = "awsvpc"
  cpu                      = 1024
  memory                   = 2048
  requires_compatibilities = ["FARGATE"]
  container_definitions    = templatefile("./prefect-ecs-agent-task.json", { prefect_api_key = "${var.prefect_api_key}", prefect_api_url = "${var.prefect_api_url}",log_group = "${aws_cloudwatch_log_group.prefect-log-group.name}" })
